...
Let a Peano curve type mapping $y^0(x)$ map the interval $[0,1]$ onto the hypercube $D_0$ of (ссылка), i.e.
...
Since the hypercube $D$ of (ссылка) belongs to the general part of the family of hypercubes (\ref{hypercubes}) (the boundary of the hypercube $D$ is highlighted in Fig \ref{fig2}), then, by introducing an additional function 
...
\begin{itemize}
\item For each scale, search for the closest object to $ω^*$; the result is a set of distances between the test object and the closest object for each scale.
