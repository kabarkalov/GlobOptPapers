...
%Баркалов
\textcolor[rgb]{1,0,0}{Алгоритмическая сложность вычисления значения функции $y^c(x)$ зависит от размерности $N$, от уровня разбиения $M$ и составляется величину $O(M\cdot N)$.}
...
%Штанюк
\textcolor[rgb]{1,0,0}{Отметим, что метод, основанный на использовании space-filling curves, может применяться не только при работа с числовыми, но и при работе с категориальными features. Значения категориальных признаков являются дискретными, поэтому для применения данного подхода требуется сопоставить каждому дискретному значению численную величину, например, из интервала [0,1]. Конкретный способ сопоставления численных значений категориальным переменным приведен в Section \ref{RealData} (в  CarEvaluation dataset).
...
\] 
%Баркалов
\textcolor[rgb]{1,0,0}{Алгоритмическая сложность вычисления всех прообразов $x^l \in [0, 1], \; 0 \leq l \leq L,$ зависит от размерности $N$, от уровня разбиения $M$, числа отображений $L$ и составляется величину $O(M\cdot N \cdot L)$.}
...
%Сысоев
\textcolor[rgb]{1,0,0}{алгоритмическая сложность операции поиска в котором имеет оценку $O(log(n))$. В качестве реализации RB-tree мы использовали контейнер set из стандартной библиотеки языка С++.}
...
%Сысоев
\textcolor[rgb]{1,0,0}{Следующий эксперимент проводился для определения зависимости времени поиска $K = 50$ соседей с ростом числа сэмплов для выбранной размерности $N = 4$. Число сэмплов менялось от 200 тыс. до 1 млн. Результаты, представленные на Fig.~\ref{figN}, показывают что соотношение между процедурой А и процедурой B остается примерно тем же, а выигрыш процедуры А у kNN-KD method составляет порядка 10 раз.}
...
%Баркалов
\textcolor[rgb]{1,0,0}{Также отметим, что данный dataset включет в себя несколько категориальных атрибутов, каждому из которых соответствуют 3-4 различных значения.
В этом случае требуется выполнить сопоставление дискретным значениями численных характеристик.
В качеcтве численных значений при приведении эксперимента были выбраны внутренние узлы равномерной сетки, покрывающей отрезок [0,1]. 
Число узлов сетки выбиралось равным числу значений признаков (3 или 4, соответствевнно). Далее, i-му значению категориального признака ставилось в соответствие значение в i-м узле сетки. }
