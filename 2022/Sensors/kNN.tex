...
%Штанюк
\item
\textcolor[rgb]{0,1,0}{\textbf{DS-WISDM} -- a dataset of dimension 3, representing accelerometer and gyroscope time-series sensor data collected from a smartphone and smartwatch \cite{ds-3}.}
...
\caption{\textcolor[rgb]{0,1,0}{Running times of the algorithms with $N = 3$, 7 million objects}\label{figW}}
\end{figure}  

%Сысоев
\textcolor[rgb]{0,1,0}{Представленные выше результаты экспериментов показывают, что даже при числе объектов в несколько миллионов времена работы рассматриваемых в статье алгоритмов остаются небольшими (в пределах миллисекунды). Таким образом, вопросы распараллеливания и реализации многопоточных версий разработанных алгоритмов в статье не рассматривались.}
