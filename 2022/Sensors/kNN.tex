...
%Баркалов
Также отметим, что данный dataset включет в себя несколько категориальных атрибутов, каждому из которых соответствуют 3-4 различных значения.
В этом случае требуется выполнить сопоставление дискретным значениями численных характеристик.
В качетсве численных значений при приведении эксперимента были выбраны внутренние узлы равномерной сетки, покрывающей отрезок [0,1]. 
