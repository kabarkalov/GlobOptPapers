...
%Штанюк
Метод, основанный на пространственных кривых, может применятся при работе с категориальными переменными. Как известно, категориальные элементы образуют множество дискретных значений. Необходимо сопоставить каждому такому значению отображение на числовой отрезок, в нашем случае на [0,1]. Расположение точек-отображений целесообразно на максимальном расстоянии друг от друга, в этом случае плотность заполняющих пространство кривых может быть уменьшена при сохранении точности обнаружения соседей.

Для иллюстрации этого подхода был взят CarEvaluation dataset(Section \ref{RealData}), который включет в себя несколько категориальных атрибутов. Эти атрибуты носят названия buying, maint, doors, persons, lug_boot, safety, каждому из которых соответствуют 3-4 значения \cite{ds-2}. 

Отображение на числовой отрезок может быть сделано с помощью равномерной сетки. Отрезок делится на фиксированное количество равных частей, равное числу значений признаков (например 3 или 4, как в примере с CarEvaluation dataset). Далее, каждому значению переменной выбирается значение либо в середине соответствующей части отрезка, либо на одном из его концов. Если необходимо, то проводится округление значения до ближайшего разряда.
