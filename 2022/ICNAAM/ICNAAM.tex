...
At present, the global optimization methods are applied to solving various problems arising in various fields of science and technology (see \cite{Pinter2006}). 
...
Among these ones, there are the problems of chemical kinetics (see \cite{Gubaydullin2021}).
...
This holds even in the case of using the efficient algorithms (for example, \cite{Paulavicius2011,Evtushenko2009,Jones2009,Paulavicius2020,Sergeyev2021}) constructing the essentially nonuniform coverages. 
...
\item[2.2] form a set $Q$ comprising of $P+1$ pairs of the kind $\left\{y_i^j, \varphi(z_i^j)\right\} $ 
\item[2.3] construct a regression model of given power $deg$ based on the data from $Q$;
