...
% Title portion
\title{Using Machine Learning for Subdivision of Parameters in Global Optimization Problems of Large Dimensionality}
...
At present, the global optimization methods are applied to solving various problems arising in various fields of science and technology. 
...
Among these ones, there are, for example, the problems of chemical kinetics \cite{Gubaydullin2021}.
...
In the present work, a special method for such a subdivision based on the analysis of the regression model of the function with respect to each variable separately has been proposed.
...
In the present work, an approach to the subdivision of the problem parameters into the two specified groups is proposed.
...
To solve the local problem (\ref{local_problem}), we used Hooke-Jeevse pattern search method (see, e.g., \cite{Kelley}). 
...
\section{SUBDIVISION OF THE PROBLEMS PARAMETERS}
...
\item use the solution found by the local method in $K$ iterations (in the present study, Hooke-Jeevse pattern search method was used) as the reference point.
\item use the solution found by the global method in $K$ iterations (in the present study, the global search algorithm was used) as the reference point.
\end{enumerate}

Step 2. For all components of $y_i, \; 1\leq i \leq N$, investigate the locality:
\begin{enumerate}
\item compute $P+1$ values of the objective function $\varphi(y)$ at the points $z_i^j = (\overline{y}_1,...,\overline{y}_{i-1},y_i^j,\overline{y}_{i+1},...,\overline{y}_N)$ where $y_i^j =  a_i + jh, \; h=(b_i-a_i)/P, \; 0\leq j \leq P$;
...
A result close to 1 shows the accurasy of the model to be high.
...
As a result of solving the Gr-L series with the preset precision of $0.01$ and the parameters of the method $K=100$, $deg=1$, and $T=0.6$, correct subdivision of he variables into the global and local ones were performed in all problems.
