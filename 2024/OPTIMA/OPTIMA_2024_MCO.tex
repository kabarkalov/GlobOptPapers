...
In the test experiments described in Section~\ref{sec:5} we used the value $r=3$, which ensured the correct solution of the entire series of problems. In applied optimization problems the value $r=3$ also provides the correct solution in most cases.
...
corresponding to boundaries of the interval $(x_{i-1},x_{i})$ belong to the Pareto set.

A significant advantage of this approach is the possibility to easily replace both the machine learning model itself and its learning rules. In the framework of the study, several variants of building models that potentially improve the factor $R_{ps}(i)$ to better solve the MCO problems.
