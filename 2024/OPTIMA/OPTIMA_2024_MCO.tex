...
Each iteration of the search consists of one trial execution. The term ``trial'' means evaluation of the minimized function $\varphi(x)$ from (\ref{eq:07}) at some point.

The first two trials are to be carried out at the boundary points of the interval $[0,1]$, i.e., at points $x^0=0$, $x^1=1$. 

Let $k$, $k \geq 2$, search iterations have been executed at trial points $x^0, x^1,\dots,x^{k-1}$ with objective function values, where $z^0, z^1,\dots,z^{k-1}$, $z^i=\varphi(x^i)$, $1 \leq i \leq k-1$. Then the choice of the new trial point $x^k$ is determined according to the following rules.
...
\end{equation}
and juxtapose to them the values $z_i=\varphi(x_i)$, $1 \leq i \leq k-1$.
...
\begin{equation}
    \label{eq:10}
		...
		\end{matrix}
\end{equation}
where $r$, $r>1$ is the \textit{reliability} parameter of the algorithm.
