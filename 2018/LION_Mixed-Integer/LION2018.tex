...
The first iteration $x^1 \in (0,1)$ is executed in an arbitrary internal point of the search interval. The choice of the next iteration point $x^{k+1}, \; k \geq 1,$ is performed according to the following rules.
\begin{itemize}
	\item Arrange the search information in the order of values of the iteration points executed earlier, i.e. 
	\[ 
	x_1 < x_2 <... <x_k.
	\]
	\item Compute a quantity $R(i)$ called a \textit{characteristic} of the interval for each interval $(x_{i-1},x_i), 1<i\leq k$.
	\item Determine the interval $(x_{t-1},x_t)$ which the maximum characteristic $R(t)=\max{\{R(i): 1\leq i \leq k}\}$ corresponds to.
	\item Compute the next iteration point $x^{k+1}$ in the interval with the maximum characteristic, i.e. $x^{k+1}\in (x_{t-1},x_t)$.
\end{itemize}

A detailed description of various variants of this algorithm is presented in \cite{ }.
...
На основе задачи (\ref{problem_i}) сформируем набор задач 
