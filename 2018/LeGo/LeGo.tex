...
specifying the number of constraints active in the minimum points have been added. The 
demonstration of the developed approach is given on the example of a known index method for 
solving the complex multiextremal optimization problems with nonlinear constraints.
\end{abstract}
...
problems with nonlinear constraints proposed originally in Ref.~\cite{Gergel2017} has been 
conducted.
...
At that, the location of the global minimum is known for the generated problems that simplifies 
evaluating the results of the numerical experiments essentially. 
in addition to the specified options, 
the following functions have been added in the current version of the generator: 
...
For example, one generator for random samples of two-dimensional test functions has been 
described in \cite{Grishagin2015}. 
%This generator produces two-dimensional functions according to the formula
%\begin{equation} \label{VAG}
%\varphi(y)= -
%\left\{\left(\sum^{7}_{i=1}\sum^{7}_{j=1}A_{ij}g_{ij}(y)+B_{ij}h_{ij}(y)\right)^2 + 
%\left(\sum^{7}_{i=1}\sum^{7}_{j=1}C_{ij}g_{ij}(y)+D_{ij}h_{ij}(y)\right)^2\right\}^{1/2},
%\end{equation}
%where $g_{ij}(y)=\sin(i\pi y_1)\sin(j\pi y_2)$, $h_{ij}(y)=\cos(i\pi y_1)\cos(j\pi y_2)$, 
%$y=(y_1,y_2)\in R^2, \; 0 \leq y_1,y_2 \leq 1$, and coefficients $A_{ij}, B_{ij}, C_{ij}, 
%D_{ij}$  are taken uniformly in the interval $[-1,1]$.
...
studying some unconstrained optimization algorithms has been described in 
\cite{Paulavicius2014,Sergeyev2015}. 

The generator GCGen (Global Constrained optimization problem Generator) which allows to 
generate the test global optimization problems with $m$ constraints based on the functions of 
this class has been proposed in Ref.~\cite{Gergel2017}. In the present paper, the rules, which 
allow formulating the constrained global optimization problems have been proposed so that:
\begin{itemize}
...
When generating the problems according to the scheme described in Ref.~\cite{Gergel2017}, 
...
constraints $S$ in the constrained minimum point, the rules are modified as follows.
\begin{enumerate}
...
As an illustration, the level lines of the objective functions and the zero-level lines of four 
...
boundary of the feasible domain, the number of active constraints is not defined.
...
parameters' variation;
	\item knowing \textit{a priori} the conditional global minimizer of the objective 
function;
	\item generating the unconditional global minimizer of the objective function out of the 
feasible domain;
	\item generating the problems with a conditional minimum point located at the boundary 
of the feasible domain;
	\item defining the number of constraints active at the optimum point.
\end{itemize}

The demonstration of the developed approach in application to well-known index method for 
solving complex multiextremal optimization problems with non-convex constraints is 
considered.

The developed approach allows generating any number of test global optimization problems 
with non-convex constraints for performing multiple computational experiments in order to 
