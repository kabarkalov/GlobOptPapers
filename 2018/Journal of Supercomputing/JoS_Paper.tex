...
\title{Parallel Algorithm for Solving Constrained Global Optimization Problems
...
\begin{itemize}
	...
	\]
\end{itemize}
The occurrence of the first violation of the constraint terminates the trial. In the case, when the point $y^k$ is a feasible one, i.e. when $x^k \in Q_{m+1},$ the trial includes the computation of the values of all problem functions and the index is accepted to be $\nu=m+1$. The pair of values
\[
\nu=\nu(x^k), \; z^k=g_\nu(y(x^k))
\]
...
Paulavi\v cius, R., Sergeyev, Y.D., Kvasov, D.E., \v Zilinskas, J. Globally-biased \textsc{Disimpl} algorithm for expensive global optimization. J. Glob. Optim. 59(2), 545--567 (2014)
