...
\begin{table}[h!]
\centering
\caption{Результаты решения задачи настройки гиперпараметров метода предсказания значений временного ряда на датасете montly beer prodaction с использованием фреймворков iOpt и Optuna}\label{tab1}
\begin{tabular}{|l|c|c|}
\hline
...
В настоящей работе были рассмотрены актуальные проблемы решения задач настройки гиперпараметров методов искусственного интеллекта (ИИ) и машинного обучения (МО).

Основная сложность таких задач заключается в довольно длительном решении соответствующей исходной задачи для каждого выбранного сочетания гиперпараметров, что накладывает ограничение на возможное количество испытываемых комбинаций.

В свою очередь принципиально новой и не менее сложной проблемой является некорректная работа исследуемых алгоритмов ИИ и МО в некоторых заранее неизвестных подобластях области изменения гиперпараметров; фактически, область допустимых сочетаний параметров является заранее неопределенной. Проблема возникновения таких недопустимых комбинаций, которая ранее не имела особого значения при <<ручной>> настройке, стала камнем преткновения для многих фреймворков автоматической настройки гиперпараметров.

Для решения исследуемого класса задач была разработана модификация одного из эффективных детерминированных методов решения задач липшицевой глобальной оптимизации -- ин\-фор\-ма\-ци\-он\-но-ста\-тис\-ти\-чес\-ко\-го алгоритма глобального поиска. Разработанный алгоритм реализован на базе фреймворка методов интеллектуальной оптимизации с открытым исходным кодом iOpt.

В статье изложено подробное описание модифицированного алгоритма (АГП-Н). Приведены результаты серийных экспериментов, демонстрирующие надёжность разработанного подхода. Продемонстрировано сравнение результатов работы АГП-Н с результатами, полученными с помощью стандартных алгоритмов оптимизации из библиотеки SciPy. Показана эффективность разработанного метода при решении исследуемого класса задач.

В качестве демонстрационных примеров были решены несколько задач настройки гиперпараметров, в которых возникает проблема недопустимых сочетаний гиперпараметров, приводящих к <<падению>> метода.

В качестве первой модельной задачи была рассмотрена задача настройки гиперпараметров классического алгоритма классификации LinearSVC. Разработанный метод успешно решил поставленную задачу, найдя лучшее сочетание гиперпараметров по сравнению со стандартными алгоритмами из библиотеки scikit-learn.

В качестве второй модельной задачи была рассмотрена задача предсказания значений временного ряда средствами фреймворка FEDOT. В ходе эксперимента также было проведено сравнение с другим известным фреймворком, применяемым для настройки гиперпараметров -- Optuna. В ходе эксперимента фреймворк iOpt продемонстрировал свое преимущество во времени решения задачи, немного уступив в качестве.
