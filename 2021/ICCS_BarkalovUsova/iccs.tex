...
%Russian
Проводимое в статье рассмотрение ограничено одномерным случаем. Обобщение нового алгоритма на многомерный случай может быть выполнено в соответствии со ставшими уже стандартными подходами из \cite{Grishagin2016,Grishagin2018}, использующими редукцию размерности на основе adaptive nested optimization scheme. 
...
%Russian
\textbf{Remark 1}. Еще раз отметим, что в соответствии с (\ref{gamma}) значение $\gamma_i+\gamma_{i-1}$ может быть равно либо 0 (обе граничных точки интервала принадлежат области непрерывности функции), либо 1 (одна из граничных точек является точкой разрыва). В первом случае формула (\ref{R_i}) будет иметь вид
\[
R(i)=\Delta_i+\frac{ (z_i-z_{i-1} )^2}{(r\mu)^2 \Delta_i }-\frac{2(z_i + z_{i-1})}{r\mu}.
\]
А во втором случае будем иметь
\[
...
%Russian
Параметр $0<q<1$ тажке задается исследователем. Значение $1 - q$ отражает априорные предполжения о том, какая доля поисковых интервалов будет иметь значения $\mu_i$ близкими к нулю, т.е. фактически характеризует степень многоэкстремальности задачи. В проведенных нами экспериментах использовалось значение $q=0.9$.
