...
%Здесь - описание алгоритма выделения локальных минимумов, с иллюстрацией на функции Шекеля.
...
Для того, чтобы использовать выделение локальных минимумов в алгоритме глобального поиска, описанном в subsection \ref{CoreGSA}, его необходимо дополнить следующими шагами. 

Step 6. Построить дерево решений на основе результатов выполненных испытаний, и получить соответствующую кусочно линейную аппроксимацию, которая каждой подобласти $D_1, D_2, ..., D_J$ сопоставляет значение $с_1, с_2, ..., с_J$.

Step 7. Для точки $x^{k+1}$ текущего испытания найти $j$ такой, что $x^{k+1} \in D_j$ и проверить выполнимость условия (\ref{LocalPointC}). Если условие (\ref{LocalPointC}) выполняется, запустить локальный поиск из точки $x^{k+1}$.
...
%Нужно еще описать исключение интервалов из глоабльно поиска, и также - новый критерий остановки.
