...
Пусть мы имеем $x^{k+1}$ - текущая точка испытания, для данной точки необходимо найти номер подобласти  $j$, такой что $x^{k+1} \in D_j$ 
Далее, необходимо проверить значения $c$ соответствующие соседним подобластям. Если выполняется условие 
\begin{equation}\label{LocalPointC}
c_{j-t_1} > … > c_{j-1} > c_{j} < c_{j+1} < c_{j+t_2}
\end{equation}
\begin{equation}\label{LocalPointT}
 t_1 + t_2 + 1  = T
\end{equation}
, где T - параметр метода, экспериментально было выбрано значение 5.
Тогда область $D_j$ является областью притяжения локального минимума.

Для того что бы внедрить данные подходы в АГП, необходимо добавить следующие шаги в алгоритм описанные в "3.1. Core global search algorithm" 

Step 6. Построить дерево решения на основе точек проведенных испытаний, $x_0<x_1<\ldots <x_{k}$, и получить соответствующую кусочно линейную аппроксимацию: каждой подобласти $D_1, D_2, ..., D_J$ соответствует значение $с_1, с_2, ..., с_J$.

Step 7. Для текущей точки испытания $x^{k+1}$  найти $j$, такое что $x^{k+1} \in D_j$ и проверить выполняется ли условие (\ref{LocalPointC}). Если условие выполняется запустить локальный метод из точки $x^{k+1}$.
