...
%Ссылки на параллельные генетические алгоритмы - простота распараллеливания
...
%Распараллеливание детерминированных алгоритмов - сложная задача
...
%Также будем предполагать, что функции задачи $g_i(y)$ определены и вычислимы только в тех точках обалсти $y \in D$, в которых выполняются ограничения
...
%исходная информация - параллельность
...
%комментарий по поводу r и eps-резервов

%комментарий про различные схемы распараллеливания - со ссылками на наши работы
...
%Априорная информация, имеющаяся при решении прикладных многоэкстремальных задач, помимо факта ограниченности разделенных разностей функций задачи может содержать дополнительные характеристики экстремума, использование которых ускоряет процесс поиска. К числу таких характеристик можно отнести некоторый априорный прогноз расположения оптимальной точки (основанный, например, на наличии прототипа оптимизируемого объекта). 
%Если же такой априорный прогноз отсутствует, 
...
%Формулы (\ref{localR}) следует использовать в правилах алгоритма на шаге вместо 
...
%Отметим, что на скорость сходимости алгоритмов глобальной оптимизации существенно влияет константа Липшица,
%поэтому столь важным является вопрос ее корректной оценки. Заниженная оценка истинной константы Липшица L может привести к потере глобального решения. В то же время завышенная оценка константы L, не соответствующая истинному поведению функции, влечет за собой медленную сходимость алгоритма к точке глобального минимума.
%Использование только глобальной информации о поведении целевой функции в ходе ее минимизации может значительно замедлить сходимость алгоритма к точке глобального минимума, т.к. могут существовать зоны области поиска, которые находятся далеко от точки глобального минимума, но локальные константы Липшица в которых существенно меньше глобальной.
%
%В работах [47, 140, 156] был предложен метод локальной настройки, преодолевающий указанные сложности и позволяющий адап-
%тивно оценивать локальные константы Липшица на всей области поиска. Балансирование локальной и глобальной информации в про-
%цессе поиска глобального минимума позволяет существенно ускорить работу алгоритма глобальной оптимизации.
%
%
%Суть предложенного в подхода состоит в следующем. В алгоритме глобального поиска предлагается использовать не единую для всей области поиска оценку константы Липшица, а строить ее оценки для каждого поискового интервала в отдельности. Поскольку получение таких оценок может происходить с достаточно большими погрешностями, то при нахождении локальных (интервальных) оценок должна учитываться и общая (глобальная) оценка константы Липшица.
%
%Приведем алгоритмическое описание предлагаемого подхода.
%Предположим, что параллельный алгоритм глобального поиска выполнил $k\geq$ итераций поиска и получены точки испытаний из (ссылка).
%
%Оценка константы Липшица   интервала (xi1,xi), 1<i<k, вычисляется в соот-ветствии со следующими выражениями:
...
%здесь - сослаться на распараллеливание с использованием блочной многошаговой схемы.
