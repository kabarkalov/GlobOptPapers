...
\end{equation}
  \item If several intervals have equal characteristics, prefer the interval with computable boundary points that has the lowest number.
...
\caption{Examples of problems No.No. 19, 25, and 35 from the first series of experiments} \label{fig6}
\end{figure}
...
The use of rule (\ref{eq25}) forms a grid of trials inside the non-computability regions that is visible clearly for Problem No. 35. At that, the trial points are denser at the boundary of the non-computable region close to potential minimum when attempting to approach this one from different sides due to the computational rules (\ref{eq22})-(\ref{eq24}) that was observed in all problems presented as the examples. 
...
\hline
 N &  Task type & Average number of trials & \begin{tabular}{@{}c@{}} {Average number of trials} \\  {in non-computability regions} \end{tabular} & Solved problems \\
\hline
