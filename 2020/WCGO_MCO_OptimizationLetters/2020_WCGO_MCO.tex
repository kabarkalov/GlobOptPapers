...
As noted earlier, the MCOlex problem at each stage (\ref{eq:9}) of the $\varepsilon$-constraint method is a global optimization problem with nonlinear constraints (\ref{eq:16}). Generally, the solution to such problems should start from the very beginning; and multiple solutions to such global optimization problems may require extensive calculations. However, the presence of search information $A_k$ from (\ref{eq:21}) allows bringing the results of previous calculations to the values of any next solved optimization problem in the scheme (\ref{eq:9}) without any time-consuming calculations of the values of the criteria $f_i (y)$, $1 \leq j \leq s$, of the problem (\ref{eq:1}) \cite{c30,c31}. 
%new text begin
This possibility of reusing search information can be briefly presented as follows.
%\begin{itemize}
%\item 
%При минимизации очередного критерия $f_l(y(x)), 1\leq l\leq s,$ получаемая поисковая информация накапливается в форме множества $A_k$ из (\ref{eq:21}) (следует отметить, что в $A_k$ наряду со значениями $z_i, 1\leq i\leq k,$ минимизируемого критерия $f_l(x)$ запоминаются и вычисленные значения критериев $f_i, 1\leq i\leq k$.
%\item 
%При переходе к минимизации следующего критерия $f_{l+1}(y(x))$ значения этого критерия $z_i, 1\leq i\leq k,$ во всех точках $x_i, 1\leq i\leq k,$ ранее выполненных итераций глобального поиска могут получены при помощи быстро выполнимых вычислений на основе запомненных значений критериев $f_i, 1\leq i\leq k$ в поисковой информации $A_k$
%\begin{equation}\label{new_equation}
%z'_i=Z(f_i), 1\leq i\leq k,
%\end{equation}
%где процедура вычисления значений $z'_i, 1\leq i\leq k$ определяется правилами (\ref{eq:9}), (\ref{eq:18})--(\ref{eq:20}) без повторных вычислениях значений критериев $f_l(y(x)), 1\leq l\leq s$.
%\end{itemize}
%new text end
...
\end{equation}
%new text begin
%При этом, как и ранее, при любых изменениях оценок  $f_j^*, \delta_j, g_j^k, 1\leq j < s, 1 \leq i \leq k,$ решение задачи (\ref{eq:30}) может быть продолжено с использованием всей имеющейся поисковой информации после быстро выполняемых вычислений новых значений $z'_i,1\leq i \leq k,$ в соответствии с процедурой (\ref{new_equation}).
%new text end
