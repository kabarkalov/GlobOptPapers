...
{\color{red}  the example taken from \cite{Book2013}})
...
\begin{figure}
  ...
  \caption{An example of the integrated function for the problem (\ref{eq:16}) using a sequential constraint calculation scheme: a) {\color{red} initial problem functions} , b) the function $F(x)$ from (\ref{eq:19})}
  \label{fig:1}
\end{figure}
...
{\color{red}
В частности, одна из модификаций связана с использованием информации о локальном поведении функций задачи \cite{Sergeyev2003}.
Условия сходимости алгоритма определяются следующей теоремой из \cite{c18}.
}
...
{\color{red} This possibility of reusing search information can be briefly presented as follows.
\begin{itemize}
\item 
При минимизации очередного критерия $f_j(y(x)), 1\leq j\leq s,$ получаемая поисковая информация накапливается в форме множества $A_k$ из (\ref{eq:21}) (следует отметить, что в $A_k$ наряду со значениями $z_i, 1\leq i\leq k,$ минимизируемого критерия $f_j(y(x))$ запоминаются и вычисленные значения векторного критерия $f(y(x_i)), 1\leq i\leq k$).
\item 
При переходе к минимизации следующего критерия $f_{j+1}(y(x))$ значения этого критерия $z'_i, 1\leq i\leq k,$ во всех точках $x_i, 1\leq i\leq k,$ ранее выполненных итераций глобального поиска могут получены при помощи быстро выполнимых вычислений на основе запомненных значений векторного критерия $f(y(x_i)), 1\leq i\leq k,$ в поисковой информации $A_k$
\begin{equation}\label{new_equation}
z'_i=Z(f(y(x_i))), 1\leq i\leq k,
\end{equation}
где процедура вычисления значений $z'_i, 1\leq i\leq k$ определяется правилами (\ref{eq:9}), (\ref{eq:18})--(\ref{eq:20}), без повторных вычислениях значений критериев $f_j(y(x)), 1\leq l\leq s$.
\end{itemize}
}
