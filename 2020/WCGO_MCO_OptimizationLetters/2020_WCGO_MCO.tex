...
\end{equation}
where for the unknown a priori values $f_j^*$, $1 \leq j < s$, can be used the estimates $z_j^k$, $1 \leq j < s$, from (\ref{eq:7}) obtained from the search information $A_k$,  and for the constants $\delta_j$, $1 \leq j<s$, we can use the values normalized to the range of constraints values, that is
...
\hline
\multicolumn{8}{c}{Search information}                                    & Reduction \\ \cline{1-8}
\multicolumn{4}{c}{not used}       & \multicolumn{4}{c}{used}             &  in \\ \cline{1-8}
Method     & Problem & Avg   & Avg   & Method     & Problem & Avg   & Avg   & iteration    \\
iter.      & solved  & HV    & DU    & iter.      & solved  & HV    & DU    & number       \\ \hline
