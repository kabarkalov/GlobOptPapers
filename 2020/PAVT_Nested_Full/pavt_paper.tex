...
the local method). The generated local subproblems will not affect each other, and the solving of these 
...
The global optimization problems arise in various fields of modern science and technology often. For 
...
in the development of new medical drugs \cite{Sulimov}. The global optimization problems arise in 
many other applications as well (see the review \cite{Pinter2006}). 
...
kind, it is required to carry out a search for the values of the unknown parameters of the model, at 
...
variables have a local effect i. e. the objective function is monoextremal with respect to these ones. 
...
The monoextremal subproblems (each corresponding to a fixed set of values of the first part of 
parameters) will be solved at the lower level. For solving the monoextremal problems, the efficient 
...
Let us consider an optimization problem of the kind
...
Accounting for such a peculiarity of the considered problem class can reduce the computational costs 
of the optimum search essentially. Indeed, according to known nested optimization scheme 
...
function value can be computed with high precision but this operation is computation-costly. 
...
Let us introduce a measure of response of the function $u(t,\omega,c)$ to the experiential data as a 
sum of squares 
\[
...
will be, generally speaking, a multiextremal one, and its solving will require considerable 
...
(\ref{global_problem}) has been developed in Lobachevskii University of Nizhnii Novgorod (UNN) 
...
objective function at every step progressively. Almost all these methods cam be represented in the 
...
functions is quite limited usually. For example, a local character of the dependence of the function on 
...
the smoothness of the minimized function. When searching for the minimum, these methods measure 
...
in the interval $[-1,1]$.
The class of functions is used for testing the global optimization algorithms often. 
...
optimization methods since the functions $G(y)$ are essentially multiextremal ones, and the functions 
$R(x)$ have a ravine structure manifested clearly.
...
trial points form a nonuniform coverage of the search domain) as well as well as the ravine character 
...
parameter $m=10$ were used; the local method employed has no additional parameters.
...
$T_{av}$, and the speed-up in time $S$ subject to the number of MPI-processes employed $p$. 
...
The results of experiments performed on the series of the optimization problems, in which a 
multiextremal part and a unimodal one can be selected demonstrated the scheme of organization of the 
