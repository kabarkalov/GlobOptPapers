...
{\color{red} The figure shows global minimizer $y^*$, $\epsilon$-reserved solution $y_\epsilon$ and the set $Y_\epsilon$ of all admissible points which are not worse than the $\epsilon$-reserved solution. }

\begin{figure}
	\center{\includegraphics[width=0.6\linewidth]{Fig1.pdf}}
	\caption{ {\color{red} An $\varepsilon$-reserved solution to a two-dimensional problem. } }
...
{\color{red} In this approach, the violation of any constraint at the point $y$ terminates the computation of the values of other functions at this point. }
...
\begin{itemize}
  ...
  \item the volume of the feasible subdomain $Q_{m+1}$ relative to the whole search domain $D$ is defined;
	...
	\item {\color{red} the number of constraints active at the global minimizer can be predefined (an inequality constraint is called active at some point if at this point the inequality turns into equality)}.
\end{itemize}	
