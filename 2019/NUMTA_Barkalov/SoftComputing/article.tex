...
In \cite{Strongin2000} it has been proven that for any limit point $x'$ of the sequence $\{x^k\}$, generated by IA while solving the problem at any $r_\nu > 1, \; 1 \leq \nu \leq m+1$, there exists a sequence
...
However, number $j(k)$ corresponds to the same interval with fixed characteristic \linebreak $R_{glob}(j(k))>0$, whereas number $t(k)$ corresponds to different intervals with different characteristics \linebreak $R_{loc}(t(k))>0$, where $R_{loc}(t(k)) \rightarrow 0$. From this we can conclude that the condition $R_{glob}(j) > \rho R_{loc}(t)$ is fulfilled for a large enough value of $k$.
}
