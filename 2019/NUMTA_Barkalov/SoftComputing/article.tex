...
\item local estimates of the constants $L_i$ in different subdomains $D_i$ of the search domain $D$ (\cite{Sergeyev1998,Kvasov2003,Sergeyev2010}, \cite{Sergeyev2016}).
...
\end{align}
\normalsize
Fig. \ref{fig:eps_reserved_solution} illustrates the concepts introduced above.
The figure shows the global minimizer $y^*$, the $\epsilon$-reserved solution $y_\epsilon$ and the set $Y_\epsilon$ of all admissible points which are not worse than the $\epsilon$-reserved solution.

\begin{figure}
	\center{\includegraphics[width=0.6\linewidth]{Fig1.pdf}}
	\caption{ An $\varepsilon$-reserved solution to a two-dimensional problem }
...
\end{align}
\normalsize
This situation is typical for the optimal design problems when some conditions for the functioning of the modeled object are not met and its other characteristics appear to be undefined. 
In this approach, the violation of any constraint at the point $y$ terminates the computation of the values of other functions at this point.
...
\end{align}
\normalsize
The issues of the numerical construction of the Peano space filling curve-type mappings and the corresponding theory were described in detail in \cite{Strongin2000,Sergeyev2013}. Note that the theoretical Peano curve $y(x)$ is a limit object and in practice is replaced by its approximation (called \textit{evolvent}) with predefined precision $2^{-l}$, where the integer number $l>1$ is a parameter (the \textit{density} of the evolvent). The evolvent density is determined by the accuracy of the possible practical implementation of the problem solution found. For  illustration, Fig. \ref{fig:evolvent} (a), (b) shows the projections of the interval [0,1] for the mapping $y(x)$ with $N=2$ and $l=4,5$, correspondingly.
\begin{figure}
	...
	\end{minipage}
	\caption{Evolvent for $N=2$ and $l=4$ (a), $l=5$ (b)}
	\label{fig:evolvent}
\end{figure}
...
\begin{itemize}
  ...
  \item the volume of the feasible subdomain $Q_{m+1}$ relative to the whole search domain $D$ is defined;
	...
	\item the number of constraints active at the global minimizer can be predefined (an inequality constraint is called active at some point if at this point the inequality turns into equality).
\end{itemize}	
...
\begin{figure}[h!]
	\begin{minipage}[h]{0.48\linewidth}
		\center{\includegraphics[width=\linewidth]{Fig14} \\ {\footnotesize(a)}}
	\end{minipage}
	\hfill
	\begin{minipage}[h]{0.48\linewidth}
		\center{\includegraphics[width=\linewidth]{Fig15} \\ {\footnotesize(b)}}
	\end{minipage}
