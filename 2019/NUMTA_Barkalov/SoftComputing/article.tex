...
\]
meaning that the interval $[x_{t-1}, x_t]$ containing the limit point $\bar{x}$ at the step $k$ is hit by the trial at the subsequent step $k+1$.

From Step 3 and Step 7 of the index algorithm follows that
\begin{equation}\label{max_gamma}
\max \{x_t-x^{k+1},x^{k+1}-x_{t-1}\} \leq \gamma (x_t-x_{t-1}),
\end{equation}
where $\gamma = \frac{r+1}{2r}<1$  and $ r=\min\{r_\nu:1\leq\nu\leq m+1\}>1$.
The contraction property (\ref{lim_delta_t}) and the property (\ref{nested_int}) for the product of all intervals (\ref{intervals_t}) are the consequences of (\ref{max_gamma}).

As shown in Lemma 1, at each iteration there exists an interval with strictly positive characteristic. Due to the rule (\ref{MaxR}) this leads to the conclusion that
...
For intervals with indexes $\nu(x_{i-1}) = \nu(x_{i}) = \nu$ from the second condition of Theorem \ref{theorem:1} and (\ref{K_leq_L}) follows that
\[
\frac{\left|z_i-z_{i-1}\right|}{\Delta_i}\leq 2L_\nu\sqrt{N+3},
\]
and in accordance with (\ref{z_estimates}) the inequalities 
\[
z_i = g_\nu(y(x_i))\geq z_\nu^*, \; \nu=\nu(x_i)
\]
holds. Then from the rules (\ref{R_1})--(\ref{R_3}) it follows that $R(i) \leq 2\Delta_i$, if $\nu(x_{i-1}) \neq \nu(x_{i})$, and
\[
R(i) \leq \Delta_i\left(1+\left(\frac{2L_\nu\sqrt{N+3}}{r_\nu\mu_\nu}\right)^2\right), 
\]
if $\nu(x_{i-1}) = \nu(x_{i}) = \nu$. These inequalities combined with (\ref{lim_delta_t}) lead us to the validity of (\ref{lim_R}).
...
\textbf{Proof}. Because for the IA with the parameters $r_{\nu}=r_{\nu}^{glob}, 1 \leq \nu \leq m+1,$ the conditions of Theorem \ref{theorem:1} are fulfilled, then any limit point $\bar{y}=y(\bar{x})$ of the trial sequence generated by the IA for the problem (\ref{problem}) satisfies the conditions (\ref{phi_bar_y}).
	...
	At every iteration of the IA-DL method there will exist such interval $[x_{j-1}, x_j]$, $j=j(k)$, that $\bar x \in [x_{j-1}, x_j]$. 
	{ \color{blue}
	According to our assumption that $\bar y = y(\bar x)$ is not the limit point, there will exist such number $\bar k$, starting from which the trial points will not fall into the interval $[x_{j-1}, x_j]$. That is, to conduct a new trial at the point $x^{k+1} \in [x_{t-1}, x_t]$ for each $k\geq \bar k$ such interval $[x_{t-1}, x_t], t=t(k),$ will be selected that $t(k) \neq j(k)$.
	}
...
\begin{align*}
	...
	& + 4\frac{z_{\nu}^{\ast}-g_{\nu}\left( y(\bar x) \right)}{r_{\nu}\mu_{\nu}} = \\
	& = { \color{red} \Delta_i\left(1-\frac{2^{3-1/N}L_{\nu}\sqrt{N+3}}{r_{\nu}\mu_{\nu}} \right)+4\frac{z_{\nu}^{\ast}-g_{\nu}\left( y(\bar x) \right)}{r_{\nu}\mu_{\nu}} } > 0.
\end{align*}
\normalsize
At $\nu=\nu(x_j ) > \nu(x_{j-1})$ and the value $r_{\nu}=r_{\nu}^{glob}$
\begin{align*}
...
\end{equation}
{\color{blue}
Indeed, both $j$ and $t$ depend from $k$, as the numbering of the trial points (by the lower index) changes at every iteration and the numbering of the intervals changes respectively.
%new text begin
However, $R_{glob}(j(k))>0$ is fixed for each $k \geq \bar k$ because $j(k)$ corresponds to the same interval for $k \geq \bar k$, whereas number $t(k)$ corresponds to different intervals with different characteristics $R_{loc}(t(k)) \rightarrow 0$. From this we can conclude that the condition $R_{glob}(j) > \rho R_{loc}(t)$ is fulfilled for a large enough value of $k$.
%new text end
}
