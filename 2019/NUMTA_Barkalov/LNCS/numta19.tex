...
%Russian
Численное сравление алгоритмов было выполнено посредством решения нескольких серий задач, порожденных генератором GKLS \cite{Gaviano2003}. Данный генератор многоэкстремальных функций широко используется для сравнения методов глобальной оптимизации (see, for example, \cite{Barkalov2018,Paulavicius2014,Sergeyev2015}). В данном исследовании были решены шесть серий по 100 задач размерностей $N = 3,4,5$. Для каждой размерности генерировались \textit{Simple} and \textit{Hard} problems, отличающиеся размерами attraction region локальных и глобального minimizer.
...
т.е. $\left\|y^k-y^\ast\right\| <\delta\left\|b-a\right\|$, где  $a$ and $b$ --- границы search domain $D$.
