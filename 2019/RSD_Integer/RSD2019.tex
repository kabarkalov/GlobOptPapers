...
problem (\ref{problem}) has been developed at University of Nizhnii Novgorod. The scheme of 
...
components: the vector $y$ falling into the hyperinterval $D$ and the vector $u$ taking a finite 
...
superimpose to each considered value $s$ to the vector $u_s$. 
...
\]
having, in general, the discontinuities of the first kind at the integer points $x_k = i, 1\leq i \leq 
S-1$.
...
Thus, we have constructed the \textit{Mixed-Integer Parallel Index Algorithm} (MIPIA) based 
on the  
reduction of the mixed-integer non-convex optimization problem to the non-convex 
...
Ref.~\cite{Barkalov2016,Gergel2016}. This approach to the use of GPU has been applied in 
...
GKLS is a well known generator of the test problems for the continuous multiextremal 
optimization~\cite{Gaviano}. It allows generating the functions of arbitrary dimensionality with 
known properties (the number of local minima, the size of their domains of attraction, the global 
minimizer, etc.).  
This generator of multiextremal functions is used for the investigations of the global 
optimization algorithms 
often~\cite{Paulavicius2014,SergeyevKvasov2015,Lebedev2015,Gergel2015}. 
...
mixed-integer problems. The rules, allowing generating the test problems of this kind consist in 
the following.

\begin{enumerate}
	\item A continuous multiextremal function $\varphi(y), \; y\in D, \; D = \left\{ a_j\leq 
...
\right)^2 + \sum_{j=1}^M \left( \frac{u_j - b_j}{b_j-a_j} \right)^2 \right],
	...
	\end{gather}
	i. e. this function has $N$ continuous and $M$ discrete parameters and achieves its 
...
Total 100 11-dimensional mixed-integer problems of this kind were generated. The accuracy of 
the search was equal to $10^{-2}$.   
...
As one can see from Table \ref{tab:2}, almost linear speedup in iterations and an essential 
...
\section{Conclusion}
%Russian
In the present paper, the results obtained in Lobachevskii State University of Nizhnii Novgorod 
...
on a series of 100 multiextremal mixed-integer test problems have been carried out 
demonstrating a good speedup of the algorithm with the use of GPU convincingly.
Thus, the parallel algorithm with the use of GPU demonstrated the time speedup $S = 182$ 
relative to the sequential one and $S = 23.7$ relative to the algorithm employing two CPU on 
the cluster node fully.
